...
class TestClient:
    ...
    def test_get_ontologies(self):
        """Test the get_ontologies method."""
        base_url = "https://example.com"
        client = Client(base_url=base_url, api_key="test-key")
        ...
        responses.add(
            responses.GET,
            urljoin(base_url, "/api/ontologies"),
            json=[
                {"ontology_name": "core_v1", "description": "Core ontology"},
                {"ontology_name": "extended_v2", "description": "Extended ontology"}
            ],
            status=200
        )
        
        # Call the method and check the response
        response = client.get_ontologies()
        assert len(response) == 2
        assert response[0]["ontology_name"] == "core_v1"
        assert response[1]["ontology_name"] == "extended_v2"
        client.close()
